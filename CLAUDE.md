...
1. **FilterBar** — left-aligned `[All | Claude Code | Codex]` segmented control plus right-aligned panel pin, General settings gear, and date range controls. The pin and General icons have a small gap and provide hover help. The General button opens settings without hiding the main panel. Pinning keeps the panel open when focus moves away or the user clicks outside; explicitly closing and reopening the panel preserves the pin until the user turns it off. Presets are `Today | 7 Days | 30 Days | All Time`; the calendar supports single-day and start/end range selection. Selecting the current day from the calendar uses the dynamic `Today` range instead of a fixed custom date, and any active date selection resets to `Today` after day rollover.
