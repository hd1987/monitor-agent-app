...
**About** — Normal-level window with the app icon (`AppIcon.icns`), name, tagline, bundle version plus release commit SHA from `AppVersion.versionWithCommit`, optional release date from `AppVersion.releaseDate`, and GitHub button. Settings, About, and update-check windows use the normal macOS window level so they move behind other apps when focus changes; only the main panel uses an elevated window level.
