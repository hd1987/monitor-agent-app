...
└── Views/
    ...
    ├── UtilityWindowDesign.swift  # Shared settings/dialog geometry, grouped surfaces, typography, and press feedback
...
**Settings** — The window opens at `820 × 600` with a `760 × 520` minimum and uses native macOS `NavigationSplitView`, a fixed full-height sidebar `List` extending beneath the traffic-light controls, grouped `Form`, sections, segmented pickers, and standard buttons instead of custom sidebar selection chrome or webpage-style cards. The sidebar has no collapse control. General, Config, and Prompt omit repeated detail titles and visually align their first content with the top edge of the first sidebar item's selection background; Config/Prompt use a `0pt` top inset, while General offsets the grouped `Form`'s additional system section spacing with a `-20pt` scroll-content inset. General preserves the established order and row descriptions, while Config/Prompt use a full-width equal-segment Claude Code / Codex tab control aligned to the editor's 20pt content insets. General subscription expiration and the main-panel date filter share locale-aligned weekdays, 28pt month navigation targets, and a stable six-row day grid with distinct today, selected, and range states while retaining single-date versus range-selection behavior. The shared footer has no top separator. Cancel closes the window. Save asks for confirmation before applying the current category, then keeps the window open and shows a quiet inline checkmark status beside the footer actions. Save only applies to the current category. Switching categories reloads from disk.

**Utility dialogs** — About, update-check, and usage-data rebuild surfaces share compact geometry, continuous corners, restrained semantic status colors, grouped detail regions, and native button hierarchy. About and update-check extend General's semantic window background through their transparent title bars without a separator. Progress, error, and success states preserve their existing actions and data semantics without elevating the windows above other apps.
