...
**Panel** (top → bottom): The borderless main panel uses one native popover material plus a separate foreground tint layer, 16pt continuous corners, and the same native macOS window shadow as the General settings window, without a separate edge border. The foreground tint keeps the panel brightness close to native macOS menu surfaces over both light and dark content while retaining restrained background depth. Main-panel secondary and tertiary text use explicit theme-aware contrast levels so labels remain readable when the panel loses focus. Content hierarchy comes from spacing, typography, and solid grouped surfaces instead of hard section dividers or stacked translucent layers. Interactive controls provide immediate press feedback with critically damped spring motion; Reduce Motion removes scaling while preserving short opacity feedback, and Reduce Transparency falls back to an opaque panel surface. Pressing `Esc` hides the panel even when it is pinned. Pressing `Tab` cycles the app filter forward, `Shift+Tab` cycles it backward, and `Enter` resets the panel below the menu-bar icon.
